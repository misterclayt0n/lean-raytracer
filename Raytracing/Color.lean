...
def hit_sphere (center : Point3) (radius : Float) (ray : Ray) : Float :=
  ...
  let c            := oc.lengthSq - radius^2
  let discriminant := half_b^2 - a*c
  if discriminant < 0 then -1.0 else (-half_b - Float.sqrt discriminant) / a

def ray_color (r : Ray) : Color :=
  let t := hit_sphere ⟨0.0, 0.0, -1.0⟩ 0.5 r
  if t > 0.0 then
    let n := (r.pointAt t - ⟨0.0, 0.0, -1.0⟩).normalize
    (0.5 : Float) * mk (n.x + 1) (n.y + 1) (n.z + 1)
  else
